...
fn test_decode_without_context_tags() {
    ...
    let (_, p) = Person::decode(&[0x30, 0x9, 
...
fn test_decode_with_optional() {
    ...
    let (_, seq) = TestSequence::decode(&[0x30, 0x8, 
...
fn test_decode_with_optional_without_context_tag() {
    ...
    let (_, seq) = TestSequence::decode(&[0x30, 0x6, 
                 OCTET_STRING_TAG_NUMBER, 0x4, 0x1, 0x2, 0x3, 0x4]).unwrap();
...
fn test_decode_with_inner_sequence() {
    ...
    let (_, seq) = SuperTestSequence::decode(&[0x30, 0x4, 0x67, 0x2, 0x30, 0x0]).unwrap();
...
fn test_decode_with_inner_sequenceof() {
    ...
    let (_, seq) = TestSequence::decode(&[0x30, 0x5, 0x30, 0x3, INTEGER_TAG_NUMBER, 0x1, 0x1]).unwrap();
